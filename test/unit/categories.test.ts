 ...
 * `migrations/0012`. The client cannot import from `src/`, so the duplication
...
describe('category taxonomy', () => {
  ...
  it('matches the values the database will accept', () => {
    const sql = readFileSync(
      new URL('../../migrations/0012_category_taxonomy_v3.sql', import.meta.url),
      'utf8',
    );
    // the CHECK in the Up migration, not the one restored by the Down
    const check = sql.slice(
      sql.indexOf('ADD CONSTRAINT'),
      sql.indexOf('-- Learned categorisations'),
    );
